"""
주봉 정배열(MA4>MA13>MA26>MA52) 매수 + 정배열 이탈 매도
+ 거래대금 10일 1500억 필터 백테스트
================================================================

매수 후보 조건:
  1) 최근 10거래일 거래대금이 "모두" 1500억원 이상 (= 10일 rolling min >= 1500억)
  2) 주봉 MA4 > MA13 > MA26 > MA52 정배열 (가장 최근 완성 주봉 기준)
매수: 위 두 조건 동시 충족 → 당일 종가 매수
매도: 보유 중 주봉 정배열이 깨지면(= MA4>13>26>52 불성립) → 당일 종가 전량 매도
      (거래대금 조건과 무관)

주봉 정배열 신호의 룩어헤드 방지:
  - 일봉 → W-FRI 리샘플링(주봉 종가 = 그 주 마지막 종가)으로 MA 계산
  - merge_asof(backward) 로 각 일자에 "직전 완성 주봉"의 정배열 여부를 매핑
    (진행 중인 주의 미완성 봉은 사용하지 않음)
...
def calc_signals(gdf: pd.DataFrame) -> pd.DataFrame:
    """거래대금 10/10 OK + 주봉 정배열 여부를 일봉에 부착."""
    ...
    aligned = (ma4 > ma13) & (ma13 > ma26) & (ma26 > ma52)  # MA52 NaN 구간은 False
    wk = pd.DataFrame({"week_end": w.index, "aligned": aligned.values})

    # 각 일자 → 직전 완성 주봉의 정배열 여부 (룩어헤드 방지)
    ...
    for date in all_dates:
        ...
        for ticker in list(positions.keys()):
            ...
            if i is None:
                continue
            row = sd["df"].iloc[i]
            if not bool(row["aligned"]):
                close_pos(ticker, row["close"], date, "정배열이탈")

        # 2) 매수 후보: 거래대금 10/10 OK + 주봉 정배열
        cands = []
        for ticker, sd in stock_data.items():
            ...
            if i is None:
                continue
            row = sd["df"].iloc[i]
            if bool(row["aligned"]) and bool(row["amt_ok"]):
...
def main():
    ...
    path = "results/backtest_weekly_align_amount.md"
