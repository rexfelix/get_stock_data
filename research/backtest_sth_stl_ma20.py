"""STH 돌파 매수 / STL 이탈 매도 + MA20 필터.

진입 조건: 기존 + 당일 종가 > MA20(20일 이동평균).
"""
...
def precompute(gdf):
    ...
    ma = gdf["close"].rolling(MA_WINDOW).mean().values
    return {"stls": stls, "sths": sths, "ma20": ma}
    ...
    for date in all_dates:
        ...
        for ticker, td in stock_data.items():
            if ticker in held:
                continue
            idx = td["date_to_idx"].get(date)
            if idx is None or idx < MA_WINDOW:
                continue
            row = td["df"].iloc[idx]
...
def main():
    ...
    header = [
        ...
        f"- 최대 {MAX_POSITIONS}종목, 1건당 {PER_TRADE/10000:.0f}만원, KOSPI200",
        "- 2023-01-01 ~ 현재",
        f"- 스킵 시그널: {skipped}건\n",
        "---\n",
    ]
    ...
    path = "results/backtest_sth_stl_ma20.md"
