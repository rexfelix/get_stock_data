"""
L.Score & KOSPI200 조건부 보유 전략 - 연도별 × 임계값별 백테스트
- L.Score = 0.6 × RS + 0.4 × V.Score
- V.Score = 0.7 × CountScore + 0.3 × AvgValueScore
  - CountScore: 최근 20일 중 거래대금 1500억+ 일수 / 20 × 99
  - AvgValueScore: min(avg_amount / 1500억, 1.0) × 99
- 임계값: L.Score(소수점 버림) >= 90~99
- 리밸런싱: 매주 1회
- 기간: 2023(9월~), 2024, 2025~
"""
...
from backtest_rs90_kospi200 import (
    ...
    get_kospi200_tickers, load_index_prices,
    calc_rs_at_date,
)
...
LOOKBACK_DAYS = 20  # 거래대금 평가 기간

PERIODS = [
    ("2023", "2023-09-25", "2023-12-29"),  # stock_all 데이터 시작일부터
...
def load_stock_all(start_date: str, end_date: str) -> pd.DataFrame:
    """stock_all 테이블에서 전종목 OHLCV+거래대금 로드"""
    query = text("""
        SELECT ticker, name, date, close, amount
        FROM stock_all
        WHERE date >= CAST(:start AS date) - interval '450 days'
...
def calc_vscore_at_date(amount_pivot: pd.DataFrame, calc_date: pd.Timestamp) -> pd.Series:
    """특정 날짜 기준 전종목 V.Score 계산"""
    valid_dates = amount_pivot.index[amount_pivot.index <= calc_date]
    if len(valid_dates) < LOOKBACK_DAYS:
        return pd.Series(dtype=float)

    recent = amount_pivot.loc[valid_dates[-LOOKBACK_DAYS:]]

    # CountScore: 최근 20일 중 1500억+ 일수 비율 × 99
    count_above = (recent >= AMOUNT_THRESHOLD).sum()
    count_score = (count_above / LOOKBACK_DAYS) * 99

    # AvgValueScore: 평균 거래대금 / 1500억 비율 (최대 1.0) × 99
    avg_amount = recent.mean()
    avg_value_score = np.minimum(avg_amount / AMOUNT_THRESHOLD, 1.0) * 99

    # V.Score = 0.7 × CountScore + 0.3 × AvgValueScore
    vscore = 0.7 * count_score + 0.3 * avg_value_score

    return vscore.dropna()


def calc_lscore_at_date(pivot: pd.DataFrame, amount_pivot: pd.DataFrame,
                         index_prices: pd.Series, calc_date: pd.Timestamp) -> pd.Series:
    """특정 날짜 기준 L.Score 계산 (소수점 버림)"""
    rs = calc_rs_at_date(pivot, index_prices, calc_date)
    vscore = calc_vscore_at_date(amount_pivot, calc_date)

    if rs.empty or vscore.empty:
        return pd.Series(dtype=float)

    # 공통 종목만
    common = rs.index.intersection(vscore.index)
    if len(common) == 0:
        return pd.Series(dtype=float)

    lscore = 0.6 * rs[common] + 0.4 * vscore[common]
    # 소수점 버림
    lscore = np.floor(lscore).astype(int)
    return lscore


def run_single_backtest(threshold, kospi200, pivot, amount_pivot, index_prices,
                        rebal_dates, name_map, start_date, end_date, lscore_cache):
    """단일 임계값 + 단일 기간 백테스트"""
    ...
    for rdate in rebal_dates:
        lscore_series = lscore_cache.get(rdate, pd.Series(dtype=float))
        if lscore_series.empty:
            port_val = cash
        ...
        target_tickers = set()
        for tk in lscore_series.index:
            if tk in kospi200 and lscore_series[tk] >= threshold:
        ...
        for tk in [t for t in list(holdings.keys()) if t not in target_tickers]:
            ...
            ls_val = int(lscore_series[tk]) if tk in lscore_series.index else "N/A"
            trade_log.append({
...
def generate_charts(all_results, all_benchmarks, output_dir):
    ...
    for pi, pname in enumerate(period_names):
        ...
        for i, r in enumerate(results):
            ...
            ax.plot(dates, returns, color=colors[i], linewidth=1.5,
                    label=f"L>={r['threshold']} ({r['total_return']:+.0f}%)", alpha=0.85)
    ...
    fig.suptitle("L.Score 임계값별 수익률 곡선 - 연도별 비교", fontsize=16, y=1.02)
    fig.tight_layout()
    ...
    ax.set_xticklabels([f"L>={t}" for t in THRESHOLDS], fontsize=10)
    ax.set_title("L.Score 임계값별 총수익률 - 연도별 비교", fontsize=14)
    ax.set_ylabel("총수익률 (%)")
    ...
    ax.set_xticks(x + width)
    ax.set_xticklabels([f"L>={t}" for t in THRESHOLDS], fontsize=10)
    ax.set_title("L.Score 임계값별 MDD - 연도별 비교", fontsize=14)
    ax.set_ylabel("MDD (%)")
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis="y")
    fig.tight_layout()
    ...
    ax.set_title("L.Score 수익/MDD 비율 히트맵 (높을수록 효율적)", fontsize=14)
...
def generate_report(all_results, all_benchmarks, elapsed):
    lines = []
    lines.append("# L.Score & KOSPI200 조건부 보유 전략 - 연도별 비교 리포트\n")
    lines.append("## L.Score 정의\n")
    lines.append("```")
    lines.append("L.Score = 0.6 × RS + 0.4 × V.Score")
    lines.append("V.Score = 0.7 × CountScore + 0.3 × AvgValueScore")
    lines.append("  CountScore   = (최근 20일 중 거래대금 1500억+ 일수 / 20) × 99")
    lines.append("  AvgValueScore = min(최근 20일 평균 거래대금 / 1500억, 1.0) × 99")
    lines.append("```\n")
    lines.append(f"- **전략**: 매주 L.Score 재계산, L.Score(소수점버림)>=임계값 & KOSPI200 종목만 동일가중 보유")
    lines.append(f"- **리밸런싱**: 매주 (첫 영업일)")
    lines.append(f"- **초기자본**: {INITIAL_CAPITAL:,}원")
    lines.append(f"- **비교 기간**: 2023년(9월~), 2024년, 2025년~현재")
    lines.append(f"- **비교 임계값**: L.Score>=90~99 (10단계)")
    lines.append(f"- **데이터**: stock_all 테이블 (거래대금=amount 컬럼, 단위: 백만원)\n")
    ...
    for pname, start, end in PERIODS:
        ...
        for r in results:
            ...
            ratio = r["total_return"] / abs(r["mdd"]) if r["mdd"] != 0 else 0
            lines.append(
                f"| L>={r['threshold']} | {r['total_return']:+.2f}% | {excess:+.2f}%p | "
...
def main():
    ...
    earliest_start = "2023-09-25"
    latest_end = datetime.today().strftime("%Y-%m-%d")

    # stock_all에서 데이터 로드 (close + amount)
    print("[2/5] stock_all 데이터 로딩...")
    stock_df = load_stock_all(earliest_start, latest_end)
    ...
    # amount 피벗 (거래대금)
    amount_pivot = stock_df.pivot_table(index="date", columns="ticker", values="amount")
    amount_pivot = amount_pivot.sort_index()  # ffill 하지 않음 (거래대금은 실제값만)
    ...
    # RS 계산을 위해 stocks 테이블에서도 데이터 로드 (1년 이전 필요)
    from backtest_rs90_kospi200 import load_stock_prices
    print("[3/5] RS 계산용 주가 데이터 로딩 (stocks 테이블)...")
    rs_stock_df = load_stock_prices(earliest_start, latest_end)
    rs_stock_df = rs_stock_df.drop_duplicates(subset=["date", "ticker"], keep="last")
    rs_pivot = rs_stock_df.pivot_table(index="date", columns="ticker", values="close")
    rs_pivot = rs_pivot.sort_index().ffill()
    print(f"      RS용 종목: {rs_pivot.shape[1]}개, 기간: {rs_pivot.index[0].date()} ~ {rs_pivot.index[-1].date()}")

    all_results = {}
    all_benchmarks = {}

    print("\n[4/5] 기간별 x 임계값별 백테스트 (L.Score>=90~99)...")
    ...
    for pname, start_date, end_date in PERIODS:
        ...
        # L.Score 사전 캐싱
        print(f"  L.Score 사전 계산 ({len(rebal_dates)}회)...")
        lscore_cache = {}
        for i, rd in enumerate(rebal_dates):
            lscore_cache[rd] = calc_lscore_at_date(rs_pivot, amount_pivot, index_prices, rd)
            if (i + 1) % 10 == 0:
                # 디버그: L.Score 분포 확인
                ls = lscore_cache[rd]
                kospi_ls = ls[ls.index.isin(kospi200)]
                if not kospi_ls.empty:
                    n90plus = (kospi_ls >= 90).sum()
                    print(f"    [{i+1}/{len(rebal_dates)}] {rd.date()}: "
                          f"KOSPI200 L>=90: {n90plus}종목, max={kospi_ls.max()}")
        ...
        for th in THRESHOLDS:
            r = run_single_backtest(th, kospi200, pivot, amount_pivot, index_prices,
                                    rebal_dates, name_map, start_date, end_date, lscore_cache)
            period_results.append(r)
            excess = r["total_return"] - bench_ret
            print(f"  L>={th:2d}: {r['total_return']:>+8.2f}% (초과 {excess:>+7.2f}%p) "
    ...
    chart_dir = os.path.join(base_dir, "charts_lscore_kospi200")
    ...
    report_path = os.path.join(results_dir, "backtest_lscore_kospi200.md")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(report)

    # 콘솔 최종 요약
    print(f"\n{'='*80}")
    print("L.Score & KOSPI200 백테스트 완료!")
    print(f"{'='*80}")
