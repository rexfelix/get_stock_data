"""
RS 낮은 임계값(80~89) & KOSPI200 조건부 보유 전략 - 연도별 비교 백테스트
- 시세 초입 진입 가능성 탐색: RS>=80~89 (기존 RS>=90~99 대비)
- 2023년, 2024년, 2025년~ 각각 독립 백테스트
- RS>=80~89 각 임계값 × 3개 기간 = 30개 백테스트
"""
...
THRESHOLDS = list(range(89, 79, -1))  # 89, 88, ..., 80


def run_single_backtest(threshold, kospi200, pivot, index_prices,
                        rebal_dates, name_map, start_date, end_date):
    """단일 임계값 + 단일 기간 백테스트"""
...
def generate_charts(all_results, all_benchmarks, output_dir):
    ...
    ax.set_title("RS 낮은 임계값(80~89) 총수익률 - 연도별 비교", fontsize=14)
    ax.set_ylabel("총수익률 (%)")
    ...
    ax.set_xticks(x + width)
    ax.set_xticklabels([f"RS>={t}" for t in THRESHOLDS], fontsize=10)
    ax.set_title("RS 낮은 임계값(80~89) MDD - 연도별 비교", fontsize=14)
    ax.set_ylabel("MDD (%)")
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis="y")
    fig.tight_layout()
    ...
    ax.set_title("수익/MDD 비율 히트맵 - RS 80~89 (높을수록 효율적)", fontsize=14)
    fig.colorbar(im, ax=ax, shrink=0.8)
    fig.tight_layout()
    fig.savefig(os.path.join(output_dir, "return_mdd_heatmap.png"), dpi=150, bbox_inches="tight")
    plt.close(fig)

    # 5) RS>=80~89 vs RS>=90~99 비교용 요약 차트 (80,85,90,95 대표값)
    print(f"      차트 저장: {output_dir}/")


def generate_report(all_results, all_benchmarks, elapsed):
    lines = []
    lines.append("# RS 낮은 임계값(80~89) & KOSPI200 조건부 보유 전략 - 연도별 비교\n")
    lines.append("## 가설: 시세 초입 진입 (RS 80~89에서 매수하면 상승 초기를 잡을 수 있는가?)\n")
    ...
    lines.append(f"- **비교 임계값**: RS>=80~89 (10단계)\n")
    ...
    # 고찰
    lines.append("## 고찰: RS 80~89 vs RS 90~99\n")
    lines.append("### 시세 초입 진입 가설 검증")
    lines.append("- RS 80~89 구간은 종목이 아직 시세 상위권에 완전히 진입하기 전 단계")
    lines.append("- **장점**: 더 많은 종목이 필터에 포함 → 분산효과 증가, 초기 상승 포착 가능")
    lines.append("- **단점**: 노이즈 종목(일시적 RS 상승 후 하락) 포함 확률 증가")
    lines.append("- RS>=90~95 대비 수익률/MDD 비율이 낮다면, 시세 확인 후 진입이 더 효율적\n")
...
def main():
    ...
    for pname, start_date, end_date in PERIODS:
        ...
        # RS 사전 캐싱
        print(f"  RS Rating 사전 계산 ({len(rebal_dates)}회)...")
        rs_cache = {}
    ...
    chart_dir = os.path.join(base_dir, "charts_rs_low_threshold")
    ...
    report_path = os.path.join(results_dir, "backtest_rs_low_threshold_yearly.md")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(report)

    # 콘솔 최종 요약
    print(f"\n{'='*80}")
    print("RS 낮은 임계값(80~89) 연도별 비교 백테스트 완료!")
    print(f"{'='*80}")
