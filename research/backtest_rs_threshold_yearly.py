"""
RS 임계값별 KOSPI200 조건부 보유 전략 - 연도별 비교 백테스트
- 2023년, 2024년, 2025년~ 각각 독립 백테스트
- RS>=90~99 각 임계값 × 3개 기간 = 30개 백테스트
"""
...
def generate_charts(all_results, all_benchmarks, output_dir):
    ...
    fig.suptitle("RS 임계값별 수익률 곡선 - 연도별 비교", fontsize=16, y=1.02)
    fig.tight_layout()
    ...
    ax.set_title("RS 임계값별 총수익률 - 연도별 비교", fontsize=14)
    ax.set_ylabel("총수익률 (%)")
    ...
    ax.set_xticks(x + width)
    ax.set_xticklabels([f"RS>={t}" for t in THRESHOLDS], fontsize=10)
    ax.set_title("RS 임계값별 MDD - 연도별 비교", fontsize=14)
    ax.set_ylabel("MDD (%)")
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis="y")
    fig.tight_layout()
    ...
    ax.set_title("수익/MDD 비율 히트맵 (높을수록 효율적)", fontsize=14)
...
def generate_report(all_results, all_benchmarks, elapsed):
    lines = []
    lines.append("# RS 임계값별 KOSPI200 조건부 보유 전략 - 연도별 비교 리포트\n")
    ...
    # 초과수익 크로스 비교
    lines.append("## 임계값별 × 기간별 초과수익(vs KOSPI) 크로스 비교\n")
    header = "| 임계값 |"
    sep = "|--------|"
    for pname, _, _ in PERIODS:
        header += f" {pname} |"
    ...
    # MDD 크로스 비교
    lines.append("## 임계값별 × 기간별 MDD 크로스 비교\n")
    header = "| 임계값 |"
    sep = "|--------|"
    for pname, _, _ in PERIODS:
        header += f" {pname} |"
    ...
    # 수익/MDD 크로스 비교
    lines.append("## 임계값별 × 기간별 수익/MDD 비율 크로스 비교\n")
    header = "| 임계값 |"
    sep = "|--------|"
    for pname, _, _ in PERIODS:
        header += f" {pname} |"
...
def main():
    ...
    # 전체 기간 데이터 로딩 (2023~현재, RS 계산에 1년 이전 필요)
    ...
    chart_dir = os.path.join(base_dir, "charts_rs_threshold_yearly")
    ...
    report_path = os.path.join(results_dir, "backtest_rs_threshold_yearly.md")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(report)

    # 콘솔 최종 요약
    print(f"\n{'='*80}")
    print("연도별 × RS 임계값별 비교 백테스트 완료!")
    print(f"{'='*80}")
